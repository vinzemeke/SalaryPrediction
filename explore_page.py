...
median_by_country = df.groupby(['Country'])["Salary"].median().sort_values(ascending=True)
...
def show_explore_page():
    ...
    st.markdown("""<hr style="height:0.5px;
    ...
    with expander1:
        ...
        if(selected == "Average Salaries"):
            fig = px.bar(mean_by_country, x = 'Country', y = "Salary", color = 'Salary', 
                title= "Average Salaries", color_discrete_sequence= px.colors.sequential.Plasma_r, height = 750)
            st.plotly_chart(figure_or_data= fig, use_container_width= True)

        else:
            fig = px.bar(median_by_country, x = 'Country', y = "Salary", color = 'Salary', 
                title = "Median Salaries", color_discrete_sequence= px.colors.sequential.Plasma_r, height = 750)
            st.plotly_chart(figure_or_data= fig, use_container_width= True)
    ...
    with expander2:
        ...
        with col1:
            selected_countries = st.multiselect("Select Countries to Compare (multiple selections allowed)", sorted(df['Country'].unique()), default = ['Australia', 'Brazil'])

        with col2:
            years = st.slider("Years of Experience", 0,30,3,1)
            education = st.selectbox("Level of Education", sorted(df['EdLevel'].unique()))

        
        compare = st.button("Compare")

        st.markdown("""<hr style="height:0.1px;
                    border:none;color:#333;
                    background-color:#123;" /> """, 
                    unsafe_allow_html=True)
        ...
        if (compare):
            ...
            with col3:
                st.caption("##### Education: {}    |    Years of Experience: {:.0f}".format(education, years))
                barchart_data = single_year_multiple_countries(selected_countries, education, years)
                fig = px.bar(barchart_data, x = "Salary", y = ("Country"), color = "Country", title= "Predicted Salary")
                st.plotly_chart(fig, use_container_width=True)


            with col4:
                st.caption("##### Predicted Salary Growth Curve")
                line_data = compare_salary_progression(selected_countries, education, years)
                fig2 = px.line(line_data, x = 'Experience', y = 'Salary', color = "Country", markers=True, title= "Predicted Salary Growth Curve")
